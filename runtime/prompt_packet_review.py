from __future__ import annotations

from collections.abc import Mapping
from dataclasses import dataclass
from typing import Any

from runtime.decision_implication_review import READY_FOR_IMPLICATION_REVIEW
from runtime.decision_review_handoff import (
    DECISION_REVIEW_HANDOFF,
    DECISION_REVIEW_HANDOFF_SCHEMA_VERSION,
    HANDOFF_BLOCKED,
    HANDOFF_INVALID,
    HANDOFF_READY,
    DecisionReviewHandoff,
)


PROMPT_PACKET_REVIEW = "PROMPT_PACKET_REVIEW"
PROMPT_PACKET_REVIEW_SCHEMA_VERSION = "1.0"
PROMPT_PACKET_REVIEW_READY = "prompt_packet_review_ready"
PROMPT_PACKET_REVIEW_BLOCKED = "blocked"
PROMPT_PACKET_REVIEW_INVALID = "invalid"
PROMPT_PACKET_BOUNDARY_WARNINGS = (
    "This prompt packet review object is not a provider request, prompt sending, provider config, secret or API key handling, dispatch, execution, approval, permission, or merge authority.",
    "No provider endpoint, live model name, credential, secret, API key, or send instruction is present.",
)
PROMPT_PACKET_REVIEW_CONSTRAINTS = (
    "prompt material remains empty",
    "human review is required before any future prompt drafting",
    "provider selection is unavailable",
    "network sending is unavailable",
)
PROMPT_PACKET_REVIEW_BOUNDARY_TEXT = (
    "note: not a provider request\n"
    "note: not a sendable prompt\n"
    "note: not an execution instruction\n"
    "note: no authority granted"
)
_MAX_COLLECTION_ITEMS = 24
_MAX_ITEM_CHARS = 512
_MAX_COLLECTION_CHARS = 4096
_INERT_FLAG_NAMES = (
    "authority_granted",
    "execution_allowed",
    "dispatch_allowed",
    "provider_call_allowed",
    "artifact_write_allowed",
    "persistence_allowed",
    "provider_request_created",
    "prompt_sent",
    "provider_config_accessed",
    "secret_accessed",
    "api_key_accessed",
    "network_accessed",
    "merge_authority_granted",
    "review_executes_anything",
)
_HANDOFF_FIELDS = {
    "object_type",
    "schema_version",
    "state",
    "source_implication_state",
    "decision_id",
    "decision_hash",
    "decision_status",
    "bundle_id",
    "bundle_hash",
    "implication_reasons",
    "blockers",
    "blocked_surfaces",
    "review_next",
    "warnings",
    "boundary_text",
    "is_review_only",
    "authority_granted",
    "execution_allowed",
    "dispatch_allowed",
    "provider_call_allowed",
    "artifact_write_allowed",
    "persistence_allowed",
    "prompt_packet_generated",
    "provider_config_mutated",
    "secret_accessed",
    "api_key_accessed",
    "handoff_executes_anything",
}


@dataclass(frozen=True)
class PromptPacketReview:
    state: str
    source_handoff_state: str
    source_implication_state: str
    decision_id: str
    decision_hash: str
    decision_status: str
    bundle_id: str
    bundle_hash: str
    prompt_material: tuple[str, ...]
    blockers: tuple[str, ...]
    warnings: tuple[str, ...]
    review_context: tuple[str, ...]
    review_next: tuple[str, ...]
    constraints: tuple[str, ...]
    boundary_text: str
    is_review_only: bool = True
    authority_granted: bool = False
    execution_allowed: bool = False
    dispatch_allowed: bool = False
    provider_call_allowed: bool = False
    artifact_write_allowed: bool = False
    persistence_allowed: bool = False
    provider_request_created: bool = False
    prompt_sent: bool = False
    provider_config_accessed: bool = False
    secret_accessed: bool = False
    api_key_accessed: bool = False
    network_accessed: bool = False
    merge_authority_granted: bool = False
    review_executes_anything: bool = False
    object_type: str = PROMPT_PACKET_REVIEW
    schema_version: str = PROMPT_PACKET_REVIEW_SCHEMA_VERSION

    def __post_init__(self) -> None:
        state = _normalize_state(self.state)
        blockers = _bounded_text_tuple(self.blockers, "blockers")
        warnings = _merge_warnings(self.warnings)
        review_context = _bounded_text_tuple(self.review_context, "review_context")
        review_next = _bounded_text_tuple(self.review_next, "review_next")
        constraints = _merge_constraints(self.constraints)
        object.__setattr__(self, "state", state)
        object.__setattr__(self, "prompt_material", ())
        object.__setattr__(self, "warnings", warnings)
        object.__setattr__(self, "review_next", review_next)
        object.__setattr__(self, "constraints", constraints)
        object.__setattr__(self, "boundary_text", PROMPT_PACKET_REVIEW_BOUNDARY_TEXT)
        object.__setattr__(self, "is_review_only", True)
        for flag_name in _INERT_FLAG_NAMES:
            object.__setattr__(self, flag_name, False)
        object.__setattr__(self, "object_type", PROMPT_PACKET_REVIEW)
        object.__setattr__(self, "schema_version", PROMPT_PACKET_REVIEW_SCHEMA_VERSION)

        if state == PROMPT_PACKET_REVIEW_READY:
            if blockers:
                raise ValueError("ready prompt packet review cannot contain blockers")
            if not review_context or not review_next:
                raise ValueError("ready prompt packet review requires bounded review context")
            object.__setattr__(self, "source_handoff_state", HANDOFF_READY)
            object.__setattr__(self, "source_implication_state", READY_FOR_IMPLICATION_REVIEW)
            object.__setattr__(self, "blockers", ())
            object.__setattr__(self, "review_context", review_context)
            for field_name in (
                "decision_id",
                "decision_hash",
                "decision_status",
                "bundle_id",
                "bundle_hash",
            ):
                object.__setattr__(
                    self,
                    field_name,
                    _required_text(getattr(self, field_name), field_name),
                )
            return

        if not blockers:
            raise ValueError("fail-closed prompt packet review requires blockers")
        object.__setattr__(self, "source_handoff_state", HANDOFF_BLOCKED if state == PROMPT_PACKET_REVIEW_BLOCKED else HANDOFF_INVALID)
        object.__setattr__(self, "source_implication_state", "")
        object.__setattr__(self, "blockers", blockers)
        object.__setattr__(self, "review_context", ())
        for field_name in (
            "decision_id",
            "decision_hash",
            "decision_status",
            "bundle_id",
            "bundle_hash",
        ):
            object.__setattr__(self, field_name, "")

    def to_dict(self) -> dict[str, Any]:
        return {
            "object_type": self.object_type,
            "schema_version": self.schema_version,
            "state": self.state,
            "source_handoff_state": self.source_handoff_state,
            "source_implication_state": self.source_implication_state,
            "decision_id": self.decision_id,
            "decision_hash": self.decision_hash,
            "decision_status": self.decision_status,
            "bundle_id": self.bundle_id,
            "bundle_hash": self.bundle_hash,
            "prompt_material": list(self.prompt_material),
            "blockers": list(self.blockers),
            "warnings": list(self.warnings),
            "review_context": list(self.review_context),
            "review_next": list(self.review_next),
            "constraints": list(self.constraints),
            "boundary_text": self.boundary_text,
            "is_review_only": self.is_review_only,
            "authority_granted": self.authority_granted,
            "execution_allowed": self.execution_allowed,
            "dispatch_allowed": self.dispatch_allowed,
            "provider_call_allowed": self.provider_call_allowed,
            "artifact_write_allowed": self.artifact_write_allowed,
            "persistence_allowed": self.persistence_allowed,
            "provider_request_created": self.provider_request_created,
            "prompt_sent": self.prompt_sent,
            "provider_config_accessed": self.provider_config_accessed,
            "secret_accessed": self.secret_accessed,
            "api_key_accessed": self.api_key_accessed,
            "network_accessed": self.network_accessed,
            "merge_authority_granted": self.merge_authority_granted,
            "review_executes_anything": self.review_executes_anything,
        }


def build_prompt_packet_review(source: object) -> PromptPacketReview:
    try:
        handoff = _canonical_handoff(source)
    except ValueError as error:
        return _failed_review(PROMPT_PACKET_REVIEW_INVALID, (str(error),), (), ())

    if handoff.state != HANDOFF_READY:
        blockers = handoff.blockers or ("source handoff is not ready for prompt packet review",)
        state = PROMPT_PACKET_REVIEW_BLOCKED if handoff.state == HANDOFF_BLOCKED else PROMPT_PACKET_REVIEW_INVALID
        return _failed_review(state, blockers, handoff.review_next, handoff.warnings)

    try:
        return PromptPacketReview(
            state=PROMPT_PACKET_REVIEW_READY,
            source_handoff_state=handoff.state,
            source_implication_state=handoff.source_implication_state,
            decision_id=handoff.decision_id,
            decision_hash=handoff.decision_hash,
            decision_status=handoff.decision_status,
            bundle_id=handoff.bundle_id,
            bundle_hash=handoff.bundle_hash,
            prompt_material=(),
            blockers=(),
            warnings=handoff.warnings,
            review_context=handoff.implication_reasons,
            review_next=handoff.review_next,
            constraints=handoff.blocked_surfaces,
            boundary_text=PROMPT_PACKET_REVIEW_BOUNDARY_TEXT,
        )
    except ValueError as error:
        return _failed_review(PROMPT_PACKET_REVIEW_INVALID, (str(error),), (), handoff.warnings)


def prompt_packet_review_to_dict(review: object) -> dict[str, Any]:
    if not isinstance(review, PromptPacketReview):
        raise ValueError("review must be a PromptPacketReview")
    return review.to_dict()


def render_prompt_packet_review(review: object) -> str:
    if not isinstance(review, PromptPacketReview):
        raise ValueError("review must be a PromptPacketReview")
    blockers = " | ".join(review.blockers)
    warnings = " | ".join(review.warnings)
    constraints = " | ".join(review.constraints)
    review_next = " | ".join(review.review_next)
    if review.state != PROMPT_PACKET_REVIEW_READY:
        return (
            f"prompt_packet_review: {review.state}\n"
            "is_review_only: true\n"
            "prompt_material: empty\n"
            f"blockers: {blockers}\n"
            f"constraints: {constraints}\n"
            f"warnings: {warnings}\n"
            f"{review.boundary_text}"
        )
    context = " | ".join(review.review_context)
    return (
        "prompt_packet_review: prompt_packet_review_ready\n"
        "is_review_only: true\n"
        "prompt_material: empty\n"
        f"source_handoff_state: {review.source_handoff_state}\n"
        f"source_implication_state: {review.source_implication_state}\n"
        f"decision_id: {review.decision_id}\n"
        f"decision_hash: {review.decision_hash}\n"
        f"decision_status: {review.decision_status}\n"
        f"bundle_id: {review.bundle_id}\n"
        f"bundle_hash: {review.bundle_hash}\n"
        f"review_context: {context}\n"
        f"review_next: {review_next}\n"
        f"constraints: {constraints}\n"
        f"warnings: {warnings}\n"
        f"{review.boundary_text}"
    )


def _canonical_handoff(source: object) -> DecisionReviewHandoff:
    if isinstance(source, DecisionReviewHandoff):
        mapping = source.to_dict()
    elif isinstance(source, Mapping):
        mapping = dict(source)
    else:
        raise ValueError("source must be a decision review handoff")
    if set(mapping) != _HANDOFF_FIELDS:
        raise ValueError("source handoff fields do not match the canonical schema")
    if mapping["object_type"] != DECISION_REVIEW_HANDOFF:
        raise ValueError("source handoff object type is invalid")
    if mapping["schema_version"] != DECISION_REVIEW_HANDOFF_SCHEMA_VERSION:
        raise ValueError("source handoff schema version is invalid")
    try:
        canonical = DecisionReviewHandoff(
            state=mapping["state"],
            source_implication_state=mapping["source_implication_state"],
            decision_id=mapping["decision_id"],
            decision_hash=mapping["decision_hash"],
            decision_status=mapping["decision_status"],
            bundle_id=mapping["bundle_id"],
            bundle_hash=mapping["bundle_hash"],
            implication_reasons=mapping["implication_reasons"],
            blockers=mapping["blockers"],
            blocked_surfaces=mapping["blocked_surfaces"],
            review_next=mapping["review_next"],
            warnings=mapping["warnings"],
            boundary_text=mapping["boundary_text"],
            is_review_only=mapping["is_review_only"],
            authority_granted=mapping["authority_granted"],
            execution_allowed=mapping["execution_allowed"],
            dispatch_allowed=mapping["dispatch_allowed"],
            provider_call_allowed=mapping["provider_call_allowed"],
            artifact_write_allowed=mapping["artifact_write_allowed"],
            persistence_allowed=mapping["persistence_allowed"],
            prompt_packet_generated=mapping["prompt_packet_generated"],
            provider_config_mutated=mapping["provider_config_mutated"],
            secret_accessed=mapping["secret_accessed"],
            api_key_accessed=mapping["api_key_accessed"],
            handoff_executes_anything=mapping["handoff_executes_anything"],
            object_type=mapping["object_type"],
            schema_version=mapping["schema_version"],
        )
    except (TypeError, ValueError) as error:
        raise ValueError("source handoff is malformed") from error
    if canonical.to_dict() != mapping:
        raise ValueError("source handoff is not canonical")
    return canonical


def _failed_review(
    state: str,
    blockers: tuple[str, ...],
    review_next: tuple[str, ...],
    warnings: tuple[str, ...],
) -> PromptPacketReview:
    return PromptPacketReview(
        state=state,
        source_handoff_state="",
        source_implication_state="",
        decision_id="",
        decision_hash="",
        decision_status="",
        bundle_id="",
        bundle_hash="",
        prompt_material=(),
        blockers=blockers,
        warnings=warnings,
        review_context=(),
        review_next=review_next,
        constraints=(),
        boundary_text=PROMPT_PACKET_REVIEW_BOUNDARY_TEXT,
    )


def _normalize_state(value: object) -> str:
    if value not in (
        PROMPT_PACKET_REVIEW_READY,
        PROMPT_PACKET_REVIEW_BLOCKED,
        PROMPT_PACKET_REVIEW_INVALID,
    ):
        raise ValueError("state must be a supported prompt packet review state")
    return str(value)


def _merge_warnings(value: object) -> tuple[str, ...]:
    warnings = list(_bounded_text_tuple(value, "warnings"))
    for warning in PROMPT_PACKET_BOUNDARY_WARNINGS:
        if warning not in warnings:
            warnings.append(warning)
    return _bounded_text_tuple(warnings, "warnings")


def _merge_constraints(value: object) -> tuple[str, ...]:
    constraints = list(_bounded_text_tuple(value, "constraints"))
    for constraint in PROMPT_PACKET_REVIEW_CONSTRAINTS:
        if constraint not in constraints:
            constraints.append(constraint)
    return _bounded_text_tuple(constraints, "constraints")


def _bounded_text_tuple(value: object, field_name: str) -> tuple[str, ...]:
    if not isinstance(value, (tuple, list)):
        raise ValueError(f"{field_name} must be a tuple or list")
    if len(value) > _MAX_COLLECTION_ITEMS:
        raise ValueError(f"{field_name} exceeds the item limit")
    normalized = tuple(_required_text(item, field_name) for item in value)
    if any(len(item) > _MAX_ITEM_CHARS for item in normalized):
        raise ValueError(f"{field_name} contains an oversized item")
    if sum(len(item) for item in normalized) > _MAX_COLLECTION_CHARS:
        raise ValueError(f"{field_name} exceeds the total size limit")
    return normalized


def _required_text(value: object, field_name: str) -> str:
    if not isinstance(value, str):
        raise ValueError(f"{field_name} must contain non-empty strings")
    normalized = value.strip()
    if not normalized:
        raise ValueError(f"{field_name} must contain non-empty strings")
    return normalized
